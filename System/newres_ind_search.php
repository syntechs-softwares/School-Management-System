...
<?php if($row_sp["Result"] == '1'):?>
      <form method="post" action="result.php?ad=<?php echo $ad_id;?>" style="float:left;height:100%;width:100%">
...
<form method="post" action="newres_ind_search.php?cl=<?php echo $cl;?>&ad=<?php echo $ad_id;?>" style="float:left;width:30%">
...
<form method="post" action="newres.php?ad=<?php echo $ad_id;?>" style="float:left;width:10%" hidden>
    <input id="All" type="submit" value="All" style="width:100%;font-size:15px;background:black;color:white;">
</form>
</div><br><br>
<form method="post" action="addres.php?tid=<?php echo $rows;?>&cl=<?php echo $cl;?>&ad=<?php echo $ad_id;?>">
<table style="width:100%">
 <tr style="background:black; color:white;border: 1px solid white;">
  ...
  <?php $name = strtoupper($row5["FName"]);
 ...
 <?php $sel4 = $con->query("SELECT * FROM `tests` WHERE `Id`='$rows'");
 while($row4=$sel4->fetch_array()){
  if($row4["OutOf"] !='0'){
    echo "value =".intval($row4["OutOf"]); 
  }
 }?> required>
 <input type="submit" name="Mark" value="Mark" style= "width:90%;font-size:20px;background:#000055;color:white;">
</form>
