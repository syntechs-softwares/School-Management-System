...
while($row_val2 = $sel_val2->fetch_array()){
    ...
    $inc3 = intval($row_val2["Spe"]);
    $inc4 = intval($row_val2["Spe_Reg"]);
    $inc5 = intval($row_val2["Spo"]);
...
<br><form method="post" action="day_report.php?ad=<?php echo $ad_id;?>" style="height:26%;width:100%">
      ...
      <form method="post" action="month_report.php?ad=<?php echo $ad_id;?>" style="height:26%;width:100%">
          <input name="submit" type="submit" value="Monthly Report" style="border:1px solid white;height:100%;font-size:30px; width:99%; color:white;background:black"><br><br><br>
      </form><br>
      <form method="post" action="more.php?ad=<?php echo $ad_id;?>" style="height:26%;width:100%">
          <input name="submit" type="submit" value="More" style="border:1px solid white;height:100%;font-size:30px; width:99%; color:white;background:#770077"><br><br><br>
      </form>
</div></center>
...
<table>
    <tr style="color:white;background:black">
      <th style="background:black;color:white;border-left: 1px none black;text-align:center">Month</th>
      <th style="background:black;color:white;border-left: 1px solid white;">Total Students</th>
      <th style="background:black;color:white;border-left: 1px solid white;">Registered</th>
      <th style="background:black;color:white;border-left: 1px solid white;">Rejoined</th>
      <th style="background:black;color:white;border-left: 1px solid white;">Stopped</th>
      <th style="background:black;color:white;border-left: 1px solid white;">Total Income</th>
      <th style="background:black;color:white;border-right: 1px solid black;border-left: 1px solid white;">Expense</th>
    </tr>
    <?php $sel2 = $con->query("SELECT * FROM `mon_rep` ORDER BY `Id` DESC");
     while($row = $sel2->fetch_array()):?>
    <tr>
    <td style="background:#4d0000;color:white;border: 1px solid white;"><?php echo "Month ".$row["Id"];?><span style="float:right"><?php echo $row["Month"]."-".$row["Year"];?></span></td>
       <td style="background:#4d0000;color:white;border: 1px solid white;"><?php echo $row["Total"]." Students";?></td>
       <td style="background:#4d0000;color:white;border: 1px solid white;">
       <?php if((intval($row["Registered"])+intval($row["Sp_Reg"])+intval($row["Spo_Reg"])) < 2){
        echo strval(intval($row["Registered"])+intval($row["Sp_Reg"])+intval($row["Spo_Reg"]))." Student";
       }else{ 
        echo strval(intval($row["Registered"])+intval($row["Sp_Reg"])+intval($row["Spo_Reg"]))." Students";}?></td>
       
       <td style="background:#4d0000;color:white;border: 1px solid white;">
       <?php if(intval($row["Activate"]) < 2){
        echo $row["Activate"]." Student";
       }else{ echo $row["Activate"]." Students";}?></td>
       
       <td style="background:#4d0000;color:white;border: 1px solid white;">
       <?php if(intval($row["Deactivate"]) < 2){
        echo $row["Deactivate"]." Student";
       }else{ echo $row["Deactivate"]." Students";}?></td>
       
       <td style="background:#4d0000;color:white;border: 1px solid white;">
       <?php $tot = 0; 
       $month = $row["Month"];
       $year = $row["Year"] ;
       $sel_rep = $con->query("SELECT * FROM `day_rep` WHERE `Month`='$month' AND `Year`='$year'");
       while($row_rep = $sel_rep->fetch_array()){
         $tot = $tot+(intval($row_rep["Registered"])*$inc2)+
          (intval($row_rep["Paid"])*$inc1)+
          (intval($row_rep["Activated"])*$inc1)+
          (intval($row_rep["Sp_Reg"])*$inc4)+
          (intval($row_rep["Sp_Paid"])*$inc3)+
          (intval($row_rep["Spo_Paid"])*$inc5)+
          (intval($row_rep["Spo_Reg"])*$inc5);
       } echo $tot." Birr";?></td>
       ...
       $exmon = $row["Month"];
       $exyea = $row["Year"];
       $sel_exp = $con->query("SELECT * FROM `expense` WHERE `Month`='$exmon' AND `Year`='$exyea'");
       while($row_exp = $sel_exp->fetch_array()){
          $tot_exp = $tot_exp + intval($row_exp["Amount"]);
       } echo $tot_exp." Birr"?></td>
    </tr>
    <?php endwhile;?>
</table>
<div>
