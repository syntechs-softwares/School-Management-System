...
<br><form method="post" action="day_report.php?ad=<?php echo $ad_id;?>" style="height:26%;width:100%">
          <input name="submit" type="submit" value="Daily Report" style="border:1px solid white;height:100%;font-size:30px; width:99%; color:white;background:black"><br><br><br>
      </form><br>
...
<tr style="color:white;background:black">
      <th style="background:black;color:white;border-left: 1px none black;">Day</th>
      <th style="background:black;color:white;border-left: 1px solid white;">Paid</th>
    ...
    <?php $sel2 = $con->query("SELECT * FROM `day_rep` ORDER BY `Id` DESC");
    ...
    <tr>
       ...
       <?php if((intval($row["Paid"])+intval($row["Sp_Paid"])+intval($row["Spo_Paid"])) < 2){
        echo strval(intval($row["Paid"])+intval($row["Sp_Paid"])+intval($row["Spo_Paid"]))." Student";
       }else{ 
        echo strval(intval($row["Paid"])+intval($row["Sp_Paid"])+intval($row["Spo_Paid"]))." Students";
        }?>
       </td>
       ...
       <?php if(intval($row["Registered"])+intval($row["Sp_Reg"])+intval($row["Spo_Reg"]) < 2){
        ...
        echo strval(intval($row["Registered"])+intval($row["Sp_Reg"])+intval($row["Spo_Reg"]))." Students";
        }?></td>
       
       <td style="background:#4d0000;color:white;border: 1px solid white;">
       <?php if(intval($row["Activated"]) < 2){
        echo $row["Activated"]." Student";
       }else{ echo $row["Activated"]." Students";}?></td>
       <td style="background:#4d0000;color:white;border: 1px solid white;">
       <?php if(intval($row["Deactivated"]) < 2){
        echo $row["Deactivated"]." Student";
       }else{ echo $row["Deactivated"]." Students";}?></td>
       <td style="background:#4d0000;color:white;border: 1px solid white;">
       <?php echo strval((intval($row["Registered"])*$inc2)+
          (intval($row["Paid"])*$inc1)+
          (intval($row["Activated"])*$inc1)+
          (intval($row["Sp_Reg"])*$inc4)+
          (intval($row["Sp_Paid"])*$inc3)+
          (intval($row["Spo_Paid"])*$inc5)+
          (intval($row["Spo_Reg"])*$inc5))." Birr";?></td>
       ...
       $exmon = $row["Month"];
       $exyea = $row["Year"];
       $sel_exp = $con->query("SELECT * FROM `expense` WHERE `Day`='$exday' AND `Month`='$exmon' AND `Year`='$exyea'");
